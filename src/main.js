...
fetch('https://pdp-movies-78.onrender.com/api/movies')
  ...
  .then(data => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      th.scope = 'row';
      th.innerHTML = `<a href="">${data[i].title}</a>`;

      let genreNameTd = document.createElement('td');
      // genreNameTd.setAttribute('colspan', '2');  
      genreNameTd.textContent = data[i].genre.name;

      let stockTd = document.createElement('td');
      stockTd.textContent = data[i].numberInStock;

      let rateTd = document.createElement('td');
      rateTd.textContent = data[i].dailyRentalRate;
